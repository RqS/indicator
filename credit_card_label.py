...
nlp = spacy.load('en')
credit_card_matcher = credit_card_extractor.load_credit_card_matcher(nlp)
nlp = pf.prep_nlp(nlp)
...
with open(input_file, 'r') as f:
    for index, sentence in enumerate(f):
        ...
        credit_card = credit_card_extractor.extract(nlp(t_simple_tokens), credit_card_matcher)
        label = pf.process_extracted(credit_card)
        if label == "NE":
            ne_f.write(sentence)
        elif label == "ONLY_P":
